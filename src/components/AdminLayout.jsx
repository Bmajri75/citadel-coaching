...
const liens = [
  ...
  { to: '/admin/coachs',      label: 'Coachs',       icon: '👤' },
...
export default function AdminLayout({ children }) {
  ...
  const pageCourante = liens.find(l => l.to === '/admin' ? pathname === '/admin' : pathname.startsWith(l.to))
    ...
    <aside className="w-56 bg-zinc-900 border-r border-zinc-800 flex flex-col h-full">
      <div className="p-5 border-b border-zinc-800 flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-zinc-600 mt-2 font-heading uppercase tracking-wide">Admin</p>
        </div>
    ...
    <div className="min-h-screen bg-zinc-950 flex">
      ...
      <div className="md:hidden fixed top-0 left-0 right-0 z-30 bg-zinc-900 border-b border-zinc-800 flex items-center justify-between px-4 h-14">
        <div className="flex items-center gap-3">
          ...
          <span className="text-sm font-heading text-zinc-300">{pageCourante?.label ?? 'Admin'}</span>
        </div>
